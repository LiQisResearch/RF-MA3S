...
def augment_xy_data_by_8_fold(problems):
    ...
    dat2 = torch.cat((k - x, y), dim=2)
    dat3 = torch.cat((x, k - y), dim=2)
    dat4 = torch.cat((k - x, k - y), dim=2)
    dat5 = torch.cat((y, x), dim=2)
    dat6 = torch.cat((k - y, x), dim=2)
    dat7 = torch.cat((y, k - x), dim=2)
    dat8 = torch.cat((k - y, k - x), dim=2)
