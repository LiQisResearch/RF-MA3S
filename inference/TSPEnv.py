...
class TSPEnv:
    ...
    def load_problems(self, batch_size, aug_factor=1):
        ...
        self.problems = torch.load("problems/MSTSP"+str(self.env_params['problem_name'])+".pth",map_location='cuda:0')
        self.problems = self.problems.to(torch.float32)
    ...
    def step(self, selected):
        ...
        for i in range(self.decoder_num):
            ...
            if done:
                ...
                reward_index=None
        if done:
            reward_index=torch.max(reward_whole_sum,dim=0).indices

        return self.step_state, reward_whole,reward_index, done

    def _get_selected_node_list(self):
...
def polar_relativisation(xy):
    ...
    xy_theta[xy_new[:,:,0]<0]=xy_theta[xy_new[:,:,0]<0]+math.pi
    temp=xy_rho.max(dim=1).values
    xy_rho_new=(xy_rho/(temp.unsqueeze(dim=1)))#norm (batch,points(disance))
    #sort
    ...
    # theta1=xy_theta_new.unsqueeze(1).repeat(1,xy_theta_new.size(dim=1),1)#(batch,points,points(angle))
    # theta2=xy_theta_new.unsqueeze(2).repeat(1,1,xy_theta_new.size(dim=1))#(batch,points(angle),points)

    angle = xy_theta_new - xy_theta_new[:,0].unsqueeze(1)#(batch,points(angle),points(angle))
    angle[angle < 0] += 2 * torch.tensor([math.pi])

    # xy_result=torch.cat((xy_rho_new.unsqueeze(dim=2),angle_new),dim=2)
