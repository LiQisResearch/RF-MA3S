...
class TSPTrainer:
    ...
    def _train_one_batch(self, batch_size,epoch):
        ...
        while not done:
            selected, prob = self.model(state,epoch)
            # shape: (batch, pomo)

            state, reward, reward_index, done = self.env.step(selected)

            # reward_whole=torch.cat((reward_whole,reward.unqueeze(0)),dim=0)
            prob_list = torch.cat((prob_list, prob[:, :, :, None]), dim=3)

        
        # Loss
        ###############################################
        advantage = reward - reward[reward_index,:,:].float().mean(dim=1, keepdims=True).unsqueeze(dim=0)
        # shape: (batch, pomo)
